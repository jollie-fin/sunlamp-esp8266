...
const char *get_weekday() {
  switch(timeClient.getDay()) {
    case 1: return PSTR("Monday");
    case 2: return PSTR("Tuesday");
    case 3: return PSTR("Wednesday");
    case 4: return PSTR("Thursday");
    case 5: return PSTR("Friday");
    case 6: return PSTR("Saturday");
    case 0: return PSTR("Sunday");
    default:return PSTR("");
  }
}
void ajaxPower()
...
*/
  ...
  server.on("/log", [](){superSerial.send();});
...
void update_light()
{
  ...
  {
    if (is_millis_increasing(g_light_shape[i].timestamp, current)
        && is_millis_increasing(current, g_light_shape[i + 1].timestamp)) {
